...
const SidebarLink = ({ to, icon: Icon, label, extraClasses = '', exact = false, onClick }) => {
    return (
        <NavLink
            ...
            className={({ isActive }) =>
                `w-full p-3 rounded-md flex items-center font-medium 
                ${isActive ? 'bg-zinc-100 dark:bg-zinc-800 dark:text-gray-300 border border-purple-900 ' : ' text-zinc-800 dark:text-zinc-200'} 
                hover:bg-zinc-200 dark:hover:bg-zinc-700 ${extraClasses}`
            }
            onClick={onClick} // Call the passed onClick function
        >
            {Icon && <Icon size={24} variant="stroke" />}
